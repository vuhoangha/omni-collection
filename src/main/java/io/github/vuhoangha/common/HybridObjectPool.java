...
public class HybridObjectPool<T> {
    ...
    private final SynchronizeObjectPool<T> popperPool;      // pool để lấy object
    private final ConcurrentObjectPool<T> pusherPool;       // pool để trả object
    ...
    public T pop() {
        ...
        if (popperPool.isEmpty()) {
            synchronized (pusherPool) {
                if (pusherPool.isFull()) {      // phải chờ cho pool này full mới lấy tại vì tránh trường hợp tổng số lượng phần tử ít hơn kích thước pool nhưng cứ lấy qua lại sẽ ko hiệu quả. Ví dụ pool có capacity 1_000_000 nhưng tối đa chỉ có 100 phần tử thành ra cứ tới ngưỡng 100 nó đã chuyển rồi, ko hiệu quả
                    while (!pusherPool.isEmpty()) {
                        popperPool.push(pusherPool.pop());
                    }
                }
            }
        }
        return popperPool.pop();
    }

    public void push(T item) {
        pusherPool.push(item);
    }
